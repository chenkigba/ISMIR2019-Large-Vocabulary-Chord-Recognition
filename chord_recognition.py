...
def chord_recognition(audio_path, lab_path, chord_dict_name='submission'):
    ...
    hmm = XHMMDecoder(template_file='data/%s_chord_list.txt' % chord_dict_name, log_input=True)
    ...
    for model_name in MODEL_NAMES:
        net = NetworkInterface(ChordNet(None), model_name, load_checkpoint=False, inference_only=True)
    ...
    log_probs = [
        np.stack([p[i] for p in probs], axis=0) for i in range(len(probs[0]))
    ]
