...
class XHMMDecoder:
    ...
    def __init_known_chord_names(self, template_file):
        ...
        f.close()
        self.known_chord_array = [((0, -1, -1, -1, -1, -1), "N")] + list(
            known_chord_array_pool.items()
        )
    ...
    def get_chord_tag_obs(self, prob_list, triad_restriction=None):
        ...
        if self.log_input:
            result_logprob = prob_triad[:, result_array[:, 0]]
        else:
        ...
        if triad_restriction is not None:
            ...
            result_logprob[
                result_array[None, :, 0] != triad_restriction[:, 0, None]
            ] = -np.inf
            result_logprob[
                result_array[None, :, 1] != triad_restriction[:, 1, None]
            ] = -np.inf
    ...
    def decode(self, prob_list, beat_arr, triad_restriction=None):
        result_names, result_logprob = self.get_chord_tag_obs(
            prob_list, triad_restriction
        )
    ...
    def __get_beat_arr(self, entry, length, use_beats, use_downbeats):
        ...
        if use_beats:
            valid_beats = [
                (int(np.round(token[0] / delta_time)), int(np.round(token[1])))
                for token in entry.beat
            ]
            valid_beats = [
                (token[0], token[1])
                for token in valid_beats
                if token[0] >= 0 and token[0] < beat_arr.shape[0]
            ]
            ...
            if use_downbeats and len(valid_beats) > 0:
                ...
                beat_arr[
                    np.array([token[0] for token in valid_beats if token[1] == 1])
                ] = 2
                    ...
                    beat_arr[
                        np.array(
                            [
                                token[0]
                                for token in valid_beats
                                if token[1] == num_beat_per_bar // 2 + 1
                            ]
                        )
                    ] = 3
        return beat_arr

    def decode_to_chordlab(
        self, entry, prob_list, use_layer_decode, use_beats=False, use_downbeats=False
    ):
        beat_arr = self.__get_beat_arr(
            entry,
            prob_list[0].shape[0],
            use_beats=use_beats,
            use_downbeats=use_downbeats,
        )
        delta_time = entry.prop.hop_length / entry.prop.sr
        decode_tags = (
            self.decode(prob_list, beat_arr)
            if not use_layer_decode
            else self.layer_decode(prob_list, beat_arr)
        )
        result = []
        last_frame = 0
        ...
        for i in range(n_frame):
            if i + 1 == n_frame or decode_tags[i + 1] != decode_tags[i]:
                result.append(
                    [last_frame * delta_time, (i + 1) * delta_time, decode_tags[i]]
                )
                last_frame = i + 1
        return result

    def decode_to_triad_chordlab(
        self, entry, prob_list, use_beats=False, use_downbeats=False
    ):
        beat_arr = self.__get_beat_arr(
            entry,
            prob_list[0].shape[0],
    ...
    def decode_to_decoration_chordlab(
        self, entry, prob_list, triad_restriction, use_beats=False, use_downbeats=False
    ):
        beat_arr = self.__get_beat_arr(
            entry,
            prob_list[0].shape[0],
...
def prob_to_spectrogram(prob_list, ref_chords):
    ...
    for arr in [result_7, result_9, result_11, result_13]:
        ...
        new_result[indices, :] = arr[
            np.arange(ref_chords.shape[0])[indices],
            (ref_chords[indices, 0] - 1).astype(int) % 12,
            :,
        ]
