...
try:
    ...
    from .mir.nn.data_decorator import (
        CQTPitchShifter,
        AbstractPitchShifter,
        NoPitchShifter,
    )
    from .mir.nn.data_provider import FramedDataProvider
    from .complex_chord import (
        Chord,
        ChordTypeLimit,
        shift_complex_chord_array_list,
        complex_chord_chop,
        enum_to_dict,
        TriadTypes,
        SeventhTypes,
        ...
        EleventhTypes,
        ThirteenthTypes,
        complex_chord_chop_list,
    )
    from .train_eval_test_split import (
        get_train_set_ids,
        get_test_set_ids,
        get_val_set_ids,
    )
...
chord_limit = ChordTypeLimit(
    triad_limit=6, seventh_limit=3, ninth_limit=3, eleventh_limit=2, thirteenth_limit=2
)
...
class ReweightedLoss(nn.Module):
        ...
        for i in range(6):
            if i == 0 or i == 1:
                self.weight[i] = torch.tensor(
                    [
                        counter[i][(j + 11) // 12]
                        for j in range(len(counter[i]) * 12 - 11)
                    ],
                    dtype=torch.float32,
                )
            else:
    ...
    def forward(self, output, tag):
        ...
        if self.triad_only:
            result = conditional_classifier_loss(
                output[0], tag[:, 0], weight=self.weight[0]
            )
        else:
            result = (
                conditional_classifier_loss(output[0], tag[:, 0], weight=self.weight[0])
                + conditional_classifier_loss(
                    output[1], tag[:, 1] + 1, weight=self.weight[1]
                )
                + conditional_classifier_loss(
                    output[2], tag[:, 2], weight=self.weight[2]
                )
                + conditional_classifier_loss(
                    output[3], tag[:, 3], weight=self.weight[3]
                )
                + conditional_classifier_loss(
                    output[4], tag[:, 4], weight=self.weight[4]
                )
                + conditional_classifier_loss(
                    output[5], tag[:, 5], weight=self.weight[5]
                )
            )
        return result
...
class ChordNet(NetworkBehavior):
    ...
    def __init__(self, cross_subpart_counter, triad_only=False):
        ...
        self.condition_linear = nn.Linear(
            self.audio_feature_block.output_size + 12 + chord_limit.triad_limit + 12,
            128,
        )
        ...
        self.output_dim2 = (
            chord_limit.seventh_limit
            + chord_limit.ninth_limit
            + chord_limit.eleventh_limit
            + chord_limit.thirteenth_limit
            + 4
        )
        self.final_fc1 = nn.Linear(
            self.hidden_dim1, self.output_dim1 + self.output_dim2
        )

        # self.loss_calc=FocalLoss(gamma=2.0)
        if cross_subpart_counter is not None:
            self.loss_reweight = ReweightedLoss(
                cross_subpart_counter,
                power=1.0,
                max_clip=1.0,
                gpu=self.use_gpu,
                triad_only=triad_only,
            )
    ...
    def forward(self, x):
        ...
        x1 = self.final_fc1(x1).reshape(
            (batch_size * seq_length, self.output_dim1 + self.output_dim2)
        )
        ...
        return (
            x1[:, : chord_limit.bass_slice_begin],
    ...
    def inference(self, x):
        ...
        output = self.feed(
            x[:, SHIFT_HIGH * SHIFT_STEP : SHIFT_HIGH * SHIFT_STEP + SPEC_DIM].view(
                (1, seq_length, SPEC_DIM)
            )
        )
...
class ChordNetCNN(NetworkBehavior):
    ...
    def __init__(self, cross_subpart_counter):
        ...
        self.final_fc1 = nn.Linear(
            self.audio_feature_block.output_size, self.output_dim1 + self.output_dim2
        )

        # self.loss_calc=FocalLoss(gamma=2.0)
        if cross_subpart_counter is not None:
            self.loss_reweight = ReweightedLoss(
                cross_subpart_counter, power=1.0, max_clip=1.0, gpu=self.use_gpu
            )
...
class ComplexChordShifter(AbstractPitchShifter):
    ...
    def pitch_shift(self, data, shift):
        return shift_complex_chord_array_list(
            complex_chord_chop_list(data, chord_limit), shift
        )
...
if __name__ == "__main__":
    ...
    train_provider.link(
        storage_x,
        CQTPitchShifter(SPEC_DIM, SHIFT_LOW, SHIFT_HIGH),
    ...
    trainer = NetworkInterface(
        ChordNet(cross_subpart_counter, triad_only=True),
        "joint_chord_net_ismir_v1.0_triad_only_reweight(1.0,1.0)_s%d" % slice_id,
    ...
    if slice_id == -1:
        trainer.train_supervised(
            ...
            val_provider,
            batch_size=12,
            learning_rates_dict={1e-3: 35, 1e-4: 25, 1e-5: 15, 1e-6: 10},
            round_per_print=10,
            round_per_save=500,
            round_per_val=-1,
            early_end_epochs=100,
            val_batch_size=1,
        )
    else:
        trainer.train_supervised(
            ...
            val_provider,
            batch_size=12,
            learning_rates_dict={1e-3: 60, 1e-4: 30, 1e-5: 30, 1e-6: 10},
            round_per_print=10,
            round_per_save=500,
            round_per_val=-1,
            early_end_epochs=5,
